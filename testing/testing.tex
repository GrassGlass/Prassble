...
\begin{forest}
 ...
 ,tikz+={
  % The bounding box of the forest environment
    ...
    \path let \p1=($(current bounding box.north)-(current bounding box.south)$) in (current bounding box.south east) node[below right] {\y1
    % \SaveHeight{\y1}
    };
  }
 ,for tree={
  ...
  [{tree}, continue, dir
    ...
    [{FileTrees\_IncludeThis}, dir]
    [{a\_file\_with\_[}, file, tikz+={\gdef\foo{Nope}}, dir]
    [{a\_file\_with\_]}, file, tikz+={\gdef\foo{Code is executed in sequential order of nodes.}}, dir]
    ...
    [{some\_folder}, dir, to be continued
      [{tmp.tex}, file, to be continued]
    ]
  ]
  \xdef\ssep{\forestoption{s sep}}
\end{forest}\par
\meaning\foo\par
