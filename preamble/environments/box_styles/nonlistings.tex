...
\ExplSyntaxOn
    % The command \modifiednewkeytheorem that saves the name of the environemnt into \g__preamble_environments_used_in_HWs_clist
        % Declaring variables
    ...
    [
        ...
        % The header
            \HWHeader{Homework \#\arabic{HWNumber}}
        % Redefinition of \the<environment> to distinguish environments (e.g. exercise) used in hw subsections vs the environments natively belonging to the lecture notes/reference text.
            \clist_map_inline:Nn \g__preamble_environments_used_in_HWs_clist 
            {
                % Save the current counter value \the#1 for the environment #1 before the hw subsection
                \int_new:c {g__preamble_HW_In_Main_inital_#1_value_int}
                \int_gset:cn {g__preamble_HW_In_Main_inital_#1_value_int} {\arabic{#1}}
                % Reset \the#1 for the hw subsection
                \setcounter{#1}{0}
                % Save the current definition of the#1 (the typeset counter) and theH#1.
                \cs_set_eq:cc {g__preamble_pagestyle_main_the#1} {the#1}
                \cs_set_eq:cc {g__preamble_pagestyle_main_theH#1} {theH#1}
    ...
    [
        ...
        \clist_map_inline:Nn \g__preamble_environments_used_in_HWs_clist
            {
            \cs_set_eq:cc {the#1} {g__preamble_pagestyle_main_the#1}
            \cs_set_eq:cc {theH#1} {g__preamble_pagestyle_main_theH#1}
            \int_set_eq:Nc \l_tmpa_int {g__preamble_HW_In_Main_inital_#1_value_int}
            \setcounter{#1}{\l_tmpa_int}
            }
    ]
\ExplSyntaxOff