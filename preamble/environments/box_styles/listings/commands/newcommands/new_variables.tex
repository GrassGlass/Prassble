...
% 4. \DisplayedToPygments{ displayed language }
  ...
  % Token lists
    % \NumberInClist{ item }{ clist }
      % An arbitrary item from the clist in \NumberInClist{ item }{ clist }, which we use to iteratively match again item = \l__listings_NumberInClist_item_of_interest_tl
    ...
    % \FileExtensionToLanguage{ file extension }
      % The displayed name from \l__listings_NameConverters_displayed_languages_clist corresponding to the given file extension 
      \tl_new:N \displayedname
      % The (Pygments-understood) language name from \l__listings_NameConverters_Pygments_languages_clist that corresponds to the given file extension
  ...
  % Clists
    % \AddFileExtensionLanguageTriple{ file extension }{ pygments language }{ displayed language } & \FileExtensionToLanguage{ file extension } & \DisplayedToPygments{ displayed language }
      ...
      % Local
        % The temporary clist of (Pygments-understood) language names that we append to \g_listings_NameConverters_file_extensions_clist 
        \clist_new:N \l__listings_AddFileExtensionLanguageTriple_new_file_extensions_clist   
        % The temporary clist of file extensions that we append to \g_listings_NameConverters_Pygments_languages_clist
        \clist_new:N \l__listings_AddFileExtensionLanguageTriple_new_Pygments_languages_clist
        % The temporary clist of displayed language names that we append to \g_listings_NameConverters_displayed_languages_clist
