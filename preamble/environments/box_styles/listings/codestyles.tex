...
% Variables
  ...
  \cs_new:Npn \__listings_autowidth_lower_eqsavebox:n #1
  {
    \bool_if:NT
      ...
      {
        % \eqsavebox does not allow for line breaks. So, we use \eqparbox here instead, which does allow for line breaks. The \sbox0 = \hbox trick allows us to obtain the same effect as using \savebox, of not having any effect on the typeset output. 
