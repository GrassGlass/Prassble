  ...
  \seq_if_in:NeTF \g__preable_Input_keywords_seq {#1}
    ...
    }
    {
      ...
      % Declaring the token lists to be used in 
        ...
        \tl_new:c {g__preamble_Input_relative_path_to_prepend_#1_tl}
        \tl_gset:cn {g__preamble_Input_relative_path_to_prepend_#1_tl} {#2}
        % <code to prepend to \Input>
        \tl_new:c {g_preamble_Input_prepend_#1_tl} 
        \tl_gset:cn {g_preamble_Input_prepend_#1_tl} {#3}
        % <code to append to \Input>
        \tl_new:c {g_preamble_Input_append_#1_tl} 
        \tl_gset:cn {g_preamble_Input_append_#1_tl} {#4}
    }
}
%
%
...
% \Input[#1]{#2} has the optional argument #1, which can prepend a file path #1 (folder_1/folder_2/.../folder_n) to the file path #2. That is, \Input[#1]{#2} = \input{#1#2}. But, #1 can also be a 'keyword' that causes the corresponding file path (defined with \KeywordforInput) to be prepended (instead of #1 itself), as well as cause some code (stored in \g_preamble_Input_prepend_#1_tl and \g_preamble_Input_append_#1_tl) to be possibly executed.
...
\tl_new:N \g_preamble_Input_prepend_tl
\tl_new:N \g_preamble_Input_append_tl
    ...
    \seq_if_in:NeTF \g__preable_Input_keywords_seq {#2} 
      {
        % If so, set the corresponding 
            ...
            % code to prepend
            % code to append
        \tl_set_eq:Nc \l__preamble_relative_path_to_prepend_tl {g__preamble_Input_relative_path_to_prepend_#2_tl}
        \tl_set_eq:Nc \g_preamble_Input_prepend_tl {g_preamble_Input_prepend_#2_tl}
        \tl_set_eq:Nc \g_preamble_Input_append_tl {g_preamble_Input_append_#2_tl}
      } 
      {
        % Otherwise, #2 is a file path that is to prepended as-is
        \tl_set:Nn \l__preamble_relative_path_to_prepend_tl {#2}
      }
    % \input the files approperiately
    \clist_set:Nn \l__preamble_files_to_be_input_clist {#3} 
    \clist_map_inline:Nn \l__preamble_files_to_be_input_clist 
        {
            \tl_use:N \g_preamble_Input_prepend_tl
            \input{\g_file_directory_to_be_used_tl/\l__preamble_relative_path_to_prepend_tl/##1}
            \tl_use:N \g_preamble_Input_append_tl
        }
  % Reset the path to prepend
    \tl_clear:N \l__preamble_relative_path_to_prepend_tl
}
\ExplSyntaxOff